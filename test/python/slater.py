...
def test_slater(use_bound_as_threshold, h5_base_name):
    beta = 100.0
    # H_loc parameters
    ...
    U_mat = U_matrix_slater(L, radial_integrals=[F0,F2,F4], basis="cubic")
    ...
    p["measure_G_l"] = True
    p["move_double"] = False
    p["use_bound_as_threshold"] = use_bound_as_threshold
    ...
    if mpi.is_master_node():
        with HDFArchive(f"{h5_base_name}.out.h5",'w') as Results:
            Results["G_tau"] = S.G_tau
            Results["G_leg"] = S.G_l

    if mpi.is_master_node():
        with HDFArchive(f"{h5_base_name}.ref.h5",'r') as Results:
            assert_block_gfs_are_close(Results["G_tau"], S.G_tau, precision=5e-6)
            assert_block_gfs_are_close(Results["G_leg"], S.G_l)

    from triqs.utility.h5diff import h5diff
    h5diff(f"{h5_base_name}.out.h5", f"{h5_base_name}.ref.h5", precision=5e-6)

test_slater(True, 'slater.use_threshold')
test_slater(False, 'slater.no_threshold')
