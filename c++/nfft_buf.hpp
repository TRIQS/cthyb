...
template<int Rank> class nfft_buf_t {
 ...
 template<std::size_t... Is> struct imfreq_product<std::index_sequence<Is...>> {
  using type = cartesian_product<decltype(Is,imfreq{})...>;
 };
...
public:
 ...
 nfft_buf_t(freq_mesh_t const& fiw_mesh, int buf_size, bool do_checks = false) :
  fiw_mesh(fiw_mesh), plan_ptr(std::make_unique<nfft_plan>()), buf_counter(0),
  buf_size(buf_size), do_checks(do_checks) {

  // Initialize NFFT plans
  all_fermion = true;
  common_factor = 1;
  triqs::tuple::for_each_enumerate(fiw_mesh.components(),
                                   [this](int r, gf_mesh<imfreq> const& m) {
  ...
  if(!all_fermion)
   nfft_indexmap = indexmaps::cuboid::domain_t<Rank>(buf_extents);
 ...
 void push_back(std::array<double, Rank> const& tau_arr, dcomplex ftau) {
  ...
  triqs::tuple::for_each_enumerate(fiw_mesh.components(),
   [&tau_arr,&sum_tau_beta,this](int r, gf_mesh<imfreq> const& m) {
 ...
 void flush() {
  ...
  for (int i = buf_counter; i < buf_size; ++i) {
   ...
   for (int r = 0; r < Rank; ++r)
    x_arr()[i * Rank + r] = -0.5 + double(i) / buf_size;
  }
  do_nfft();
  buf_counter = 0;
 }
