...
namespace cthyb {
  ...
  class g4_measures_t {
    ...
    g4_measures_t(const G_tau_t &_Delta_tau, const gf_struct_t &gf_struct, const solve_parameters_t &params) : gf_struct(gf_struct), params(params) {
      ...
      // Measure all blocks
      if (g4_blocks_to_measure.empty()) {
        for (auto const &bn1 : gf_struct) {
          for (auto const &bn2 : gf_struct) { g4_blocks_to_measure.emplace(bn1.first, bn2.first); }
        }
      } else { // Check the blocks we've been asked to measure
        for (auto const &bn : g4_blocks_to_measure) {
      ...
      for (auto const &block_pair : g4_blocks_to_measure) {
        ...
        int s2 = params.measure_g4_block_order == AABB ? s1 : s3;
        int s4 = params.measure_g4_block_order == AABB ? s3 : s1;
