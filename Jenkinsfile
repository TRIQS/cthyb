...
for (int i = 0; i < dockerPlatforms.size(); i++) {
  ...
  platforms[platform] = { -> node('linux && docker && triqs') {
    stage(platform) { timeout(time: 1, unit: 'HOURS') { ansiColor('xterm') {
      ...
      /* build and tag */
      def args = '-DTRACE_DEBUG_CHECKS=ON'
      if (platform == documentationPlatform)
        args = "-DBuild_Documentation=1 ${args}"
...
for (int i = 0; i < osxPlatforms.size(); i++) {
  ...
  platforms["osx-${platform}-complex"] = { -> node('osx && triqs') {
    stage("osx-${platform}-complex") { timeout(time: 1, unit: 'HOURS') { ansiColor('xterm') {
      def srcDir = pwd()
      ...
      dir(buildDir) { withEnv(platformEnv[1].collect { it.replace('\$BREW', env.BREW) } + [
        ...
        sh "cmake $srcDir -DCMAKE_INSTALL_PREFIX=$installDir -DTRIQS_ROOT=$triqsDir -DTRACE_DEBUG_CHECKS=ON -DHybridisation_is_complex=ON -DLocal_hamiltonian_is_complex=ON"
