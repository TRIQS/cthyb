...
// -----------------------------------------------------------------------------
move_global::move_global(std::string const& name, indices_map_t const& substitution_map, qmc_data& data,
 ...
 if (identity)
  std::cerr << "WARNING: global move '" << name << "' changes no operator indices, therefore is useless." << std::endl;
}
