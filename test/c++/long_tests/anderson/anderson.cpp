...
#include <applications/impurity_solvers/ctqmc_krylov/operator.hpp>
#include <applications/impurity_solvers/ctqmc_krylov/fundamental_operator_set.hpp>
...
int main(int argc, char* argv[]) {
  ...
  if(argc<3) return 2;
  bool block_g = boost::lexical_cast<bool>(argv[1]);
  bool use_qn = boost::lexical_cast<bool>(argv[2]);
  ...
  if (block_g) {
    ...
    my_map[std::make_tuple("up")] = std::make_pair(0,0);
    my_map[std::make_tuple("down")] = std::make_pair(0,1);
  ...
  if(rank==0){
    ...
    filename_ss << "anderson.krylov";
    if(block_g) filename_ss << "_block";
    if(use_qn) filename_ss << "_qn";
    filename_ss << ".h5";
    H5::H5File G_file(filename_ss.str(),H5F_ACC_TRUNC);
    if (block_g) {
      h5_write(G_file,"G_up",G[0]);
      h5_write(G_file,"G_down",G[1]);
    } else {
      h5_write(G_file,"G_tot",G[0]);
    }
  }
