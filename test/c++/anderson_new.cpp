...
int main(int argc, char* argv[]) {
  ...
  //g0_w(om_) << triqs::gfs::inverse( om_ - mu - ( V*V / (om_ - epsilon) + V*V / (om_ + epsilon)) );
  //solver.g0t_view()[0] = triqs::gfs::inverse_fourier(g0_w);
  g0_w(om_) << om_ - mu - ( V*V / (om_ - epsilon) + V*V / (om_ + epsilon));
  solver.g0t_view()[0] = triqs::gfs::inverse_fourier( triqs::gfs::inverse( g0_w ) );
