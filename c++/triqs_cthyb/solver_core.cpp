...
namespace triqs_cthyb {
  ...
  void solver_core::solve(solve_parameters_t const &solve_parameters_) {
    ...
    for (auto bl : range(gf_struct.size())) {
      ...
      for (auto [i, j] : product_range(bl_size, bl_size)) {
        ...
        if (i == j && max_imag > 1e-10) {
          std::cout << "WARNING: max(abs(imag(S.Delta_tau[" << bl << "][" << i << ", " << j << "]))) = " << max_imag << " setting to zero.\n";
    ...
    if (auto_warmup) {
      ...
      if (!converged) {
        if (params.verbosity >= 1) std::cout << "  WARNING: warmup did not converge after " << params.max_warmup_cycles << " cycles\n";
      } else {
        if (params.verbosity >= 2) std::cout << "  mean k = " << mean_k << " -> converged\n";
      }
    ...
    if (auto_length_cycle) {
      ...
      auto stop_cb = [&]() -> bool {
        ...
        if (elapsed > next_print) {
          next_print = 1.25 * elapsed + 2.0;
          if (params.verbosity >= 2) std::cout << "  tau_ac = " << tau << " (" << calib_count << " cycles)\n";
        }
      ...
      if (params.verbosity >= 2) std::cout << "  tau_ac = " << tau_raw << " -> length_cycle = " << effective_length_cycle << "\n";
    ...
    if (params.measure_G2_iw_nfft)
      qmc.add_measure(measure_G2_iw_nfft<G2_channel::AllFermionic>{G2_iw_nfft, data, G2_measures}, "G2_iw nfft fermionic measurement");
    ...
    if (params.measure_G2_iw_ph_nfft)
      qmc.add_measure(measure_G2_iw_nfft<G2_channel::PH>{G2_iw_ph_nfft, data, G2_measures}, "G2_iw_ph nfft particle-hole measurement");
    ...
    if (params.measure_O_tau) {
      ...
      qmc.add_measure(measure_O_tau_ins{O_tau, data, n_tau, O1, O2, params.measure_O_tau_min_ins, qmc.get_rng()}, "O_tau insertion measure");
    }
