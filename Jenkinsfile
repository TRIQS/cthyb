  ...
  platforms[platform] = { -> stage(platform) {
    timeout(time: 1, unit: 'HOURS') {
      node('docker') {
        ...
        """
        /* build and tag */
        def img = docker.build("flatironinstitute/cthyb:${env.BRANCH_NAME}-${env.STAGE_NAME}")
...
\${BUILD_LOG,maxLines=60}
    """,
    to: 'hstrand@flatironinstitute.org, nils.wentzell@gmail.com, dsimon@flatironinstitute.org',
    recipientProviders: [
