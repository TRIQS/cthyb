...
// c++2py.py ../c++/solver_core.hpp -p --members_read_only -m cthyb -o cthyb -I ../cbuild/c++ -I../c++
...
template <> struct py_converter<solve_parameters_t> {
 static PyObject *c2py(solve_parameters_t const & x) {
  ...
  PyDict_SetItemString( d, "measure_g4_tau"        , convert_to_python(x.measure_g4_tau));
  PyDict_SetItemString( d, "measure_g4_iw"         , convert_to_python(x.measure_g4_iw));
  PyDict_SetItemString( d, "measure_g4_iw_pp"      , convert_to_python(x.measure_g4_iw_pp));
  PyDict_SetItemString( d, "measure_g4_iw_ph"      , convert_to_python(x.measure_g4_iw_ph));
  PyDict_SetItemString( d, "measure_g4_l_pp"       , convert_to_python(x.measure_g4_l_pp));
  PyDict_SetItemString( d, "measure_g4_l_ph"       , convert_to_python(x.measure_g4_l_ph));
  PyDict_SetItemString( d, "measure_g4_block_order", convert_to_python(x.measure_g4_block_order));
  PyDict_SetItemString( d, "measure_g4_blocks"     , convert_to_python(x.measure_g4_blocks));
  PyDict_SetItemString( d, "measure_g4_n_tau"      , convert_to_python(x.measure_g4_n_tau));
  PyDict_SetItemString( d, "measure_g4_n_bosonic"  , convert_to_python(x.measure_g4_n_bosonic));
  PyDict_SetItemString( d, "measure_g4_n_fermionic", convert_to_python(x.measure_g4_n_fermionic));
  PyDict_SetItemString( d, "measure_g4_n_l"        , convert_to_python(x.measure_g4_n_l));
  PyDict_SetItemString( d, "nfft_buf_sizes"        , convert_to_python(x.nfft_buf_sizes));
 ...
 static solve_parameters_t py2c(PyObject *dic) {
  ...
  _get_optional(dic, "measure_g4_tau"        , res.measure_g4_tau           ,false);
  _get_optional(dic, "measure_g4_iw"         , res.measure_g4_iw            ,false);
  _get_optional(dic, "measure_g4_iw_pp"      , res.measure_g4_iw_pp         ,false);
  _get_optional(dic, "measure_g4_iw_ph"      , res.measure_g4_iw_ph         ,false);
  _get_optional(dic, "measure_g4_l_pp"       , res.measure_g4_l_pp          ,false);
  _get_optional(dic, "measure_g4_l_ph"       , res.measure_g4_l_ph          ,false);
  _get_optional(dic, "measure_g4_block_order", res.measure_g4_block_order   ,AABB);
  _get_optional(dic, "measure_g4_blocks"     , res.measure_g4_blocks        ,(std::set<std::pair<std::string,std::string>>{}));
  _get_optional(dic, "measure_g4_n_tau"      , res.measure_g4_n_tau         ,10);
  _get_optional(dic, "measure_g4_n_bosonic"  , res.measure_g4_n_bosonic     ,30);
  _get_optional(dic, "measure_g4_n_fermionic", res.measure_g4_n_fermionic   ,30);
  _get_optional(dic, "measure_g4_n_l"        , res.measure_g4_n_l           ,20);
  _get_optional(dic, "nfft_buf_sizes"        , res.nfft_buf_sizes           ,(std::map<std::string,int>{}));
 ...
 static bool is_convertible(PyObject *dic, bool raise_exception) {
  ...
  std::vector<std::string> ks, all_keys = {"h_int","n_cycles","partition_method","quantum_numbers","length_cycle","n_warmup_cycles","random_seed","random_name","max_time","verbosity","move_shift","move_double","use_trace_estimator","measure_g_tau","measure_g_l","measure_g4_tau","measure_g4_iw","measure_g4_iw_pp","measure_g4_iw_ph","measure_g4_l_pp","measure_g4_l_ph","measure_g4_block_order","measure_g4_blocks","measure_g4_n_tau","measure_g4_n_bosonic","measure_g4_n_fermionic","measure_g4_n_l","nfft_buf_sizes","measure_pert_order","measure_density_matrix","use_norm_as_weight","performance_analysis","proposal_prob","move_global","move_global_prob","imag_threshold"};
  ...
  _check_optional <bool                                          >(dic, fs, err, "measure_g4_tau"        , "bool");
  _check_optional <bool                                          >(dic, fs, err, "measure_g4_iw"         , "bool");
  _check_optional <bool                                          >(dic, fs, err, "measure_g4_iw_pp"      , "bool");
  _check_optional <bool                                          >(dic, fs, err, "measure_g4_iw_ph"      , "bool");
  _check_optional <bool                                          >(dic, fs, err, "measure_g4_l_pp"       , "bool");
  _check_optional <bool                                          >(dic, fs, err, "measure_g4_l_ph"       , "bool");
  _check_optional <cthyb::block_order                            >(dic, fs, err, "measure_g4_block_order", "cthyb::block_order");
  _check_optional <std::set<std::pair<std::string, std::string> >>(dic, fs, err, "measure_g4_blocks"     , "std::set<std::pair<std::string, std::string> >");
  _check_optional <int                                           >(dic, fs, err, "measure_g4_n_tau"      , "int");
  _check_optional <int                                           >(dic, fs, err, "measure_g4_n_bosonic"  , "int");
  _check_optional <int                                           >(dic, fs, err, "measure_g4_n_fermionic", "int");
  _check_optional <int                                           >(dic, fs, err, "measure_g4_n_l"        , "int");
  _check_optional <std::map<std::string, int>                    >(dic, fs, err, "nfft_buf_sizes"        , "std::map<std::string, int>");
